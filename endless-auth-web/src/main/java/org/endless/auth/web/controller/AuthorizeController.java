package org.endless.auth.web.controller;
...
import org.endless.auth.service.interfac.OAuthService;
import org.endless.auth.web.AuthConstants;
		...
		try {
			...
			if (!oAuthService.checkClientId(authzRequest.getClientId())) {
				OAuthResponse oAuthResponse = OAuthASResponse.errorResponse(HttpServletResponse.SC_BAD_REQUEST)//
						...
						.setErrorDescription(AuthConstants.INVALID_CLIENT_DESCRIPTION).buildJSONMessage();
			...
			if (!subject.isAuthenticated()) {
				if (!login(subject, request)) {// 登录失败时跳转到登陆页面
					model.addAttribute("client", clientService.findByClientId(authzRequest.getClientId()));
					return "oauth2login";
				}
			}
